...
def run():
    ...
    teacher_student_model = tsModel.TeacherStudentNetwork(
        in_channels=config.in_channels,
        out_channels=config.out_channels,
    ...
    # Ensure teacher's weights are loaded
    teacher_student_model.teacher.load_state_dict(torch.load(config.teacher_model_path, weights_only=True))
